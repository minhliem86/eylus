                    ...
                    <fieldset class="form-group">
                        <div class="form-group row">
                            <div class="custom-control custom-checkbox custom-control-inline">
                                <input type="checkbox" id="seo_checking" name="seo_checking" {!! !$inst->metable()->isEmpty() ? 'checked' : null  !!} class="custom-control-input ">
                                <label class="custom-control-label" for="seo_checking"><b>CẤU HÌNH SEO</b></label>
                            </div>
                        </div>
                        <div class="seo-container">
                            @if(!$inst->metable()->isEmpty())
                                @foreach($inst->metable()->all() as $item_meta )
                            <div class="form-group row">
                                <label for="" class="col-md-3 col-form-label">Keywords</label>
                                <div class="col-md-9">
                                    {!! Form::text('keywords',old($item_meta->meta_keywords), ['class' => 'form-control']) !!}
                                </div>
                            </div>
                            <div class="form-group row">
                                <label for="" class="col-md-3 col-form-label">Description</label>
                                <div class="col-md-9">
                                    {!! Form::text('description',old($item_meta->meta_description), ['class' => 'form-control']) !!}
                                </div>
                            </div>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-9">
                                    ...
                                    <span class="input-group-btn">
                                        ...
                                        {{Form::hidden('meta_img',old($item_meta->meta_img), ['class'=>'form-control', 'id'=>'thumbnail_meta' ])}}

                                    </div>
                                    <img id="holder" style="margin-top:15px;max-height:100px;" src="{{asset('public/uploads/'.$item_meta->meta_img)}}">
                                </div>
                            </div>
                                @endforeach
                            @endif
